 ...
 * DESCRIPTION: Input module provides simple parsing functions (i.e. fields, numbers, ...)
 */
 ...
 */
#define PARSER_IS_EOL(text_line) ((**text_line)==EOL)
...
int input_text_parse_integer(
  ...
  if (**text_line=='0' && (*(*text_line+1)=='x' || *(*text_line+1)=='X')) {
    *text_line+=2;
    if (!IS_HEX_DIGIT(**text_line)) return -1;
    // Parse Value
    while (IS_HEX_DIGIT(**text_line)) {
    ...
    if (**text_line==PLUS) {
      PARSER_NEXT_CHAR(text_line);
    } else if (**text_line==MINUS) {
      positive = false;
      PARSER_NEXT_CHAR(text_line);
    }
    // Parse number
    if (!IS_DIGIT(**text_line)) return -1;
    while (IS_DIGIT(**text_line)) {
...
int input_text_parse_extended_uint64(
  ...
  // Textual
  if (strcasecmp(argument,"all")==0) { *value = UINT64_MAX; return 0; }
  if (strcasecmp(argument,"inf")==0) { *value = UINT64_MAX; return 0; }
  if (strcasecmp(argument,"infinite")==0)  { *value = UINT64_MAX; return 0; }
  if (strcasecmp(argument,"unlimited")==0) { *value = UINT64_MAX; return 0; }
  if (strcasecmp(argument,"none")==0) { *value = 0; return 0; }
  if (strcasecmp(argument,"zero")==0) { *value = 0; return 0; }
  if (strcasecmp(argument,"null")==0) { *value = 0; return 0; }
  // Number
...
int input_text_parse_extended_int64(
    ...
    int64_t* const value) {
  // Textual
  if (strcasecmp(argument,"all")==0) { *value = INT64_MAX; return 0; }
  if (strcasecmp(argument,"inf")==0) { *value = INT64_MAX; return 0; }
  if (strcasecmp(argument,"infinite")==0) { *value = INT64_MAX; return 0; }
  if (strcasecmp(argument,"-inf")==0) { *value = INT64_MIN; return 0; }
  if (strcasecmp(argument,"-infinite")==0) { *value = INT64_MIN; return 0; }
  if (strcasecmp(argument,"none")==0) { *value = 0; return 0; }
  if (strcasecmp(argument,"zero")==0) { *value = 0; return 0; }
  // Number
...
bool input_text_parse_extended_bool(char* const argument) {
  if (argument==NULL) {
    ...
    if (strcmp(argument,"true")==0 ||
        ...
        strcmp(argument,"allow")==0 ||
        strcmp(argument,"enabled")==0) {
      return true;
    } else {
      return false;
    }
